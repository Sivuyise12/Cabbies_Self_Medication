...
namespace ImfuyoRanch.Controllers
{
    public class PaymentController : Controller
    {
        ...
        public ActionResult OnceOff(string id)
        {
            ...
            var itemID = db.Order_Items.Where(x => x.Order_id == id).ToList();
            foreach (var item in itemID)
            {
                var items = db.Items.Find(item.item_id);
                items.QuantityInStock -= item.quantity;
                db.Entry(items).State = System.Data.Entity.EntityState.Modified;
                db.SaveChanges();
            }
            //SendMail(id);
            EmailSender.sendOrderTrackEmail(id);
            //var order=
            // Merchant Details
