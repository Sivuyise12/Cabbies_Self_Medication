...
namespace ImfuyoRanch.Services
{
    public class Order_Service
    {
        ...
        public OrderDetailModel GetOrderDetail(string order_id)
        {
            try
            {
                string shipping_method = "Collect at warehouse", payment_method = "Paid";
        ...
        public void MarkOrderAsPacked(string order_id)
        {
            ...
            if (ModelsContext.Shipping_Addresses.Where(p => p.Order_ID == order_id) != null)
            {
                ...
                EmailSender.sendOrderTrackEmail(order_id);
                //order tracking
        ...
        public void AssignToDriver(string order_id)
        {
            var order = GetOrder(order_id);
            order.packed = true;
            if (ModelsContext.Shipping_Addresses.Where(p => p.Order_ID == order_id) != null)
        ...
        public void MarkOrderAsDeliverd(string order_id)
        {
            var order = GetOrder(order_id);
            order.packed = true;
            if (ModelsContext.Shipping_Addresses.Where(p => p.Order_ID == order_id) != null)
            ...
            DriverDeliverd(order_id);
        }
        public void MarkOrderAsReceived(string order_id)
        {
            var order = GetOrder(order_id);
            order.packed = true;
            if (ModelsContext.Shipping_Addresses.Where(p => p.Order_ID == order_id) != null)
        ...
        public void DriverDeliverd(string order_id)
        {
            try
            {
                var assignDriver = ModelsContext.AssignEmployees.Where(x => x.Order_ID == order_id).FirstOrDefault();
                if (assignDriver != null)
                {
                    assignDriver.Status = "Deliverd";
                    EmailSender.sendOrderTrackEmail(order_id);

                    ModelsContext.Entry(assignDriver).State = System.Data.Entity.EntityState.Modified;
                    ModelsContext.SaveChanges();
                }
        ...
        public void AddOrder(Customer customer)
        {
            try
            {
                string orderId = GenerateOrderNumber(10);
        ...
        public void AddOrderItems(Order order, List<Cart_Item> items)
        {
            ...
            {
                //var salesReport = ModelsContext.SalesReports.Where(y=>y.ItemCode==item.item_id).FirstOrDefault();
                ...
                //salesReport.NumberOfOrders += 1;
                //ModelsContext.Entry(salesReport).State = System.Data.Entity.EntityState.Modified;
                ModelsContext.Order_Items.Add(x);
                ModelsContext.SaveChanges();
            }

            var orderReport = new OrderReports();
            orderReport.OrderId = order.Order_ID;
            orderReport.CustomerEmail = order.Email;
            orderReport.DateCreated = DateTime.Now;
            orderReport.OrderAmount = orderPrice;
            ModelsContext.OrderReports.Add(orderReport);
            ModelsContext.SaveChanges();
        }
